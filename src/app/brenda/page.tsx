...
function GoogleReviewsWidget({ reviews: data }: {
  ...
  return (
    <div style={{ position: "relative" }}>
      {/* Google-branded card trigger — compact on mobile */}
      <motion.button
        onClick={() => setOpen(!open)}
        className="google-review-trigger"
        style={{
          display: "flex",
          alignItems: "center",
          gap: 8,
      ...
      >
        ...
        <div style={{ textAlign: "left" }}>
          <div style={{ display: "flex", alignItems: "center", gap: 4 }}>
            <span style={{ fontSize: 15, fontWeight: 700, color: "var(--text)" }}>{data.rating}</span>
            <StarRating rating={data.rating} size={10} />
          </div>
          <span style={{ fontSize: 10, color: "var(--text-muted)" }}>{data.reviewCount} reviews</span>
        </div>
        <motion.span
